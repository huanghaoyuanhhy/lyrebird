...
// tokenKind classifies one lexed SQL token. The shape mirrors
// translate/pg's lexer (the house pattern for hand-rolled SQL), but the
// keyword and operator sets are the catalog dialect's: joins, CASE and the
// regex operators catalog queries use, with no vector-distance operators.
type tokenKind int
...
// keywords are the reserved words the catalog grammar consumes. Unquoted
// identifiers fold to lower case (standard PG catalog semantics — catalog
// objects are lowercase); a double-quoted identifier rescues any colliding
// name, which is how pgjdbc's quoted ORDER BY aliases ("TABLE_SCHEM") keep
// their case.
var keywords = map[string]bool{
	"SELECT": true, "FROM": true, "WHERE": true, "AND": true, "OR": true,
	"NOT": true, "IN": true, "IS": true, "NULL": true, "AS": true,
	"ASC": true, "DESC": true, "LIMIT": true, "OFFSET": true,
	"TRUE": true, "FALSE": true, "ORDER": true, "BY": true,
...
func lex(src string) ([]token, error) {
	...
	for i < len(src) {
		...
		case c == '$' && i+1 < len(src) && isDigit(src[i+1]):
			...
			i += j - i
		case c == '$':
			return nil, errf(i, "dollar-quoted strings are not supported")
		default:
			op := ""
...
func lexQuoted(src string, start int, quote byte) (text string, n int, err error) {
	...
	for {
		if i >= len(src) {
			return "", 0, errf(start, "unterminated %c-quoted literal", quote)
		}
		if src[i] == quote {
